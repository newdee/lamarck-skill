...
try {
  ...
  for (const l of lines) {
    try { const o = JSON.parse(l); if (o && o.session === sid) mine.push(o); } catch { /* tolerate corrupt lines */ }
  }
  ...
  const reason =
    ...
    `Do this now WITHOUT reloading SKILL.md. For each pending entry in ${p('data', 'pending.jsonl')} belonging to this session, using only real in-context evidence (<=5 lines each): ` +
    ...
    `For active skills: append one JSON line per entry to ${p('data', 'ledger.jsonl')} with fields {ts,session,skill,ver,trigger_fit,gaps,outcome,friction,note} (carry ver over from the pending record); if a reusable lesson emerged, append it to ${p('data', 'learnings')}${path.sep}<skill>.md; for corrected/failed entries also distill a regression case {essence,expect,src} into ${p('data', 'replays')}${path.sep}<skill>.jsonl. ` +
    `Maintain maturity.json accordingly (clean streak of stability.streak, default 10, promotes to stable). Then remove this session's lines from data${path.sep}pending.jsonl. ` +
