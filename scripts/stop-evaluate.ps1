...
try {
    ...
    $reason = "lamarck LIGHT loop (trigger: mode=$mode, needed=$needed): $($mine.Count) skill invocation(s) from this session (session_id=$sid) await evaluation: [$($names -join ', ')]. " +
        ...
        "Append one JSON line per entry to $root\data\ledger.jsonl with fields {ts,session,skill,ver,trigger_fit,gaps,outcome,friction,note} (carry ver over from the pending record); if a reusable lesson emerged, append it to $root\data\learnings\<skill>.md; for corrected/failed entries also distill a regression case {essence,expect,src} into $root\data\replays\<skill>.jsonl; then remove this session's lines from data\pending.jsonl. " +
