...
try {
    ...
    $reason = "lamarck LIGHT loop (trigger: mode=$mode, needed=$needed): $($mine.Count) skill invocation(s) from this session (session_id=$sid) await evaluation: [$($names -join ', ')]. " +
        ...
        "First consult $root\data\maturity.json: for skills marked stable, each entry needs only a glance - if this turn shows no user correction or anomaly, ledger one line {outcome:'stable-skip',ver:...} and increment the streak, except every Nth call (stability.sample, default 5) still gets the full evaluation; any correction, gap, ver change or novel scenario wakes the skill back to active. For active skills: append one JSON line per entry to $root\data\ledger.jsonl with fields {ts,session,skill,ver,trigger_fit,gaps,outcome,friction,note} (carry ver over from the pending record); if a reusable lesson emerged, append it to $root\data\learnings\<skill>.md; for corrected/failed entries also distill a regression case {essence,expect,src} into $root\data\replays\<skill>.jsonl. Maintain maturity.json accordingly (clean streak of stability.streak, default 10, promotes to stable). Then remove this session's lines from data\pending.jsonl. " +
