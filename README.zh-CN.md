...
2. **生产遥测**(按设计自动积累):每次调用带目标 skill 基因哈希,每笔被
   接受的编辑都有前后窗口。这个 loop 天然 LLM 密集(评估 → 诊断 → 变异 →
   评委),所以窗口先用两条 LLM 扭不动的通道度量:**代码从 transcript 数出来
   的客观信号**——工具调用数、工具报错、非零退出码、重试、用户轮次——以
   JSON 注入、模型只许原样抄;以及**用户纠正率**,纠正判定必须附用户原句。
   评委(成对盲评,难判升 3 评委多数)只在客观通道不反对时才有投票权:编辑
   后客观信号劣化,一票否决 "better" 并出回滚提案。全部 verify 结论落账;
   `/lamarck report` 硬数字在前。
...
| `isolation` | `inline` | 评估在哪跑:`inline` 在本回合上下文里(免费,协议会进上下文);`subagent` 派一个子代理从磁盘读协议、从实时落盘的 transcript 读证据,主上下文只收一行(多一次模型调用) |
...
| `stats` | 调用频次、纠正率、gap 排行——数字来自 `scripts/report.js --brief`,模型只叙述 |
| `report [skill]` | 基于 `scripts/report.js` 的进化叙事:各版本客观信号均值与 delta(劣化自动标出)、纠正率、保留/回滚的编辑、replay 通过率 |
...
stable skill 降为一眼扫描、`isolation: subagent` 把整个评估搬出主上下文、
每次注入带 ephemeral 标记让 harness compaction 优先丢弃。

**不开 agent 看进展**——证据报告是纯脚本;模型从不算数字,只叙述:

```
node scripts/report.js --since 30d          # markdown,逐 skill 逐版本
node scripts/report.js --since 90d --json   # 存档作证据
```

每个数字都能从账本逐行复核;唯一的时钟是账本自己的最新时间戳,同样的数据
永远产出同样的字节。
