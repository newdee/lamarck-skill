...
- Unknown encoding after all four attempts → stop, ask the user; do not
  transcode lossily.
- Quoted fields containing newlines (multiline cells) → parse with a real
  CSV reader, never by splitting on line breaks; if the row count from
  naive line-splitting disagrees with the parser's, trust the parser and
  note the multiline-cell count in the summary.
